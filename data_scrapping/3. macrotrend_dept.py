...
# 부채총계, 부채비율 데이터 가져오기
url = "https://www.macrotrends.net/stocks/charts/VZ/verizon/debt-equity-ratio"
...
data.to_csv("../stock_data_basicStat/dept.csv", sep=",", index=False)


# 유동비율 가져오기
# Current Ratio
url = "https://www.macrotrends.net/stocks/charts/VZ/verizon/current-ratio"
res = requests.get(url = url)
soup = BeautifulSoup(res.text, "lxml")
...
# 폴더 먼저 생성할 것 'stock_data_basicStat'
data = pd.DataFrame(row_list, columns=name_list)
data.to_csv("../stock_data_basicStat/current_ratio.csv", sep=",", index=False)
